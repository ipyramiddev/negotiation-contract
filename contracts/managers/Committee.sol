...
contract Committee is OwnableInitializable, NativeMetaTransaction {
    ...
    constructor(address _owner, address[] memory _members) {
        // EIP712 init
        _initializeEIP712("Unicial Collection Committee", "1");
        // Ownable init
        _initOwnable();
     ...
     */
    function setMembers(address[] calldata _members, bool[] calldata _values)
        external
        onlyOwner
    {
        require(
    ...
    function manageCollection(
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            _collectionManager.manageCollection(
                _forwarder,
                _collection,
                _data[i]
            );
