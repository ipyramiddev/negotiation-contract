...
contract CollectionManager is OwnableInitializable, NativeMetaTransaction {
    ...
    event AcceptedTokenSet(
        IERC20 indexed _oldAcceptedToken,
        IERC20 indexed _newAcceptedToken
    );
    event CommitteeSet(
        address indexed _oldCommittee,
        address indexed _newCommittee
    );
    ...
    event RaritiesSet(
        IRarities indexed _oldRarities,
        IRarities indexed _newRarities
    );
    ...
    constructor(
        ...
        // EIP712 init
        _initializeEIP712("Unicial Collection Manager", "1");
        // Ownable init
        _initOwnable();
     ...
     */
    function setAcceptedToken(IERC20 _newAcceptedToken) public onlyOwner {
        require(
            address(_newAcceptedToken) != address(0),
     ...
     */
    function setCommittee(address _newCommittee) public onlyOwner {
        require(
            _newCommittee != address(0),
     ...
     */
    function setCommitteeMethods(
        bytes4[] memory _methods,
        bool[] memory _values
    ) public onlyOwner {
        uint256 length = _methods.length;
        require(
            length > 0 && length == _values.length,
     ...
     */
    function setRarities(IRarities _newRarities) public onlyOwner {
        require(
            address(_newRarities) != address(0),
    ...
    function createCollection(
        ...
        IERC721CollectionFactory _factory,
        bytes32 _salt,
        string memory _name,
        ...
        require(
            address(_forwarder) != address(this),
        ...
        for (uint256 i = 0; i < _items.length; i++) {
            ...
            IRarities.Rarity memory rarity = rarities.getRarityByName(
                item.rarity
            );
        ...
        bytes memory data = abi.encodeWithSelector(
            IERC721Collection.initialize.selector,
            _name,
            _symbol,
            _baseURI,
            _creator,
        ...
        (bool success, ) = _forwarder.forwardCall(
            address(_factory),
            abi.encodeWithSelector(
                _factory.createCollection.selector,
                _salt,
                data
            )
        );
        require(success, "CollectionManager#createCollection: FORWARD_FAILED");
    }
    ...
    function manageCollection(
        ...
        require(
            allowedCommitteeMethods[method],
        ...
        (success, res) = address(_collection).staticcall(
            abi.encodeWithSelector(_collection.COLLECTION_HASH.selector)
        );
        require(
            success &&
                abi.decode(res, (bytes32)) == keccak256("Unicial Collection"),
            "CollectionManager#manageCollection: INVALID_COLLECTION"
        );

        (success, ) = _forwarder.forwardCall(address(_collection), _data);
        require(success, "CollectionManager#manageCollection: FORWARD_FAILED");
    }
}
