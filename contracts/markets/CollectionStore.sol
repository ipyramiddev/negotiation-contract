...
contract CollectionStore is OwnableInitializable, NativeMetaTransaction {
    ...
    constructor(
        ...
        // EIP712 init
        _initializeEIP712("Unicial Collection Store", "1");
        // Ownable init
        _initOwnable();
    ...
    function buy(ItemToBuy[] memory _itemsToBuy) external {
        ...
        for (uint256 i = 0; i < _itemsToBuy.length; i++) {
            ...
            require(
                amountOfItems == itemToBuy.prices.length,
            ...
            for (uint256 j = 0; j < amountOfItems; j++) {
                ...
                (uint256 itemPrice, address itemBeneficiary) = getItemBuyData(
                    collection,
                    itemId
                );
                require(
     ...
     */
    function getItemBuyData(IERC721Collection _collection, uint256 _itemId)
        public
        view
        returns (uint256, address)
    {
        (, , , uint256 price, address beneficiary, , ) = _collection.items(
            _itemId
        );
