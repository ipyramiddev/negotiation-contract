    ...
    NativeMetaTransaction
{
    ...
    bytes32 public constant COLLECTION_HASH = keccak256("Unicial Collection");
    uint8 public constant ITEM_ID_BITS = 40;
    uint8 public constant ISSUED_ID_BITS = 216;
    ...
    modifier onlyCreator() {
        require(_isCreator(), "onlyCreator: CALLER_IS_NOT_CREATOR");
        _;
    }
     ...
     */
    function setMinters(address[] calldata _minters, bool[] calldata _values)
        external
        onlyCreator
    {
        require(
    ...
    function setItemsMinters(
        ...
        require(
            _itemIds.length == _minters.length &&
                _minters.length == _values.length,
     ...
     */
    function setManagers(address[] calldata _managers, bool[] calldata _values)
        external
        onlyCreator
    {
        require(
    ...
    function setItemsManagers(
        ...
        require(
            _itemIds.length == _managers.length &&
                _managers.length == _values.length,
    ...
    function editItemsData(
        ...
        require(isEditable, "editItemsData: COLLECTION_NOT_EDITABLE");
        ...
        for (uint256 i = 0; i < _itemIds.length; i++) {
            ...
            require(
                !isApproved ||
                    keccak256(abi.encode(item.metadata)) ==
    ...
    function issueTokens(
        ...
        require(
            _beneficiaries.length == _itemIds.length,
    ...
    function _issueToken(
        ...
        // Check issuance
        require(
            currentIssuance <= item.maxSupply,
    ...
    function rescueItems(
        ...
        // Check lengths
        require(
            _itemIds.length == _contentHashes.length &&
